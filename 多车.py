...
# STEP 1. 用正弦函数调整位置，准备倒车入库
y1 = 180 + np.arange(Points[1][1] - Points[0][1],-1)
x1 = np.zeros_like(y1)
for i in range(len(x1)):
    x1[i] = (Points[1][0] + Points[0][0]) / 2 - (Points[1][0] - Points[0][0]) / 2 * np.cos((np.pi/(Points[1][1] - Points[0][1]))*y1[i])


# STEP 2. 走圆形，完成倒库动作
phi2 = np.arange(int(Points[1][2] / np.pi * 180 - 90), int(Points[2][2] / np.pi * 180 - 90), -1)
#-------- 计算半径 ----------#
R2 = 25
#-------- R2即为半径 ---------#
x2 = np.zeros_like(phi2)
y2 = np.zeros_like(phi2)
for i in range(len(phi2)):
    x2[i] = Points[2][0] + R2 * np.cos(phi2[i] * np.pi / 180)
    y2[i] = 250 - Points[1][1] - R2 * np.sin(phi2[i] * np.pi / 180)

# STEP 3. 直线倒车进库
x3 = np.arange(Points[2][0], Points[3][0], -1)
y3 = 250 - Points[2][1] * np.ones_like(x3)


# STEP 4. 直线出库
x4 = np.arange(Points[3][0], Points[4][0], -1)
y4 = 250 - Points[3][1] * np.ones_like(x4)


# STEP 5. 正弦函数出
x5 = np.arange(Points[5][0] - Points[4][0]) + Points[4][0]
y5 = np.zeros_like(x5)
for i in range(len(x5)):
    y5[i] = 250 - ((Points[5][1] + Points[4][1]) / 2 - (Points[5][1] - Points[4][1]) / 2 * np.cos((np.pi/(Points[5][0] - Points[4][0]))*x5[i]))
